...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        'border-input dark:bg-input/30 dark:active:bg-input/50 bg-background flex h-10 flex-row items-center justify-between gap-2 rounded-md border py-2 shadow-sm shadow-black/5 sm:h-9', searchField !== "true" && 'px-3',
        Platform.select({
     ...
     { searchField !== 'true' && ( <Icon as={ChevronDown} aria-hidden={true} className="text-muted-foreground size-4" />) }
    </SelectPrimitive.Trigger>
  );
}
